...
const DEFAULT_PROJECT_NAME = "enpilink-project";
...
const HELP_MESSAGE = `Usage: enpilink create [path] [options]

⛰ enpilink v${version} - the fullstack framework for building MCP Apps
...
Non-interactive usage:
  ...
  Example: enpilink create my-app --yes`;
...
export async function init(args: string[] = process.argv.slice(2)) {
  ...
  prompts.intro(
    `\x1b[1;36m⛰  Welcome to enpilink v${version} \x1b[22m- the fullstack framework for building MCP Apps\x1b[0m`,
  );
...
\x1b[2m${output}\x1b[0m`);
      prompts.log.error("Try manually: `npx skills add enpitech/enpilink`.");
    }
  }
...
${scriptCommand(pm, "deploy")}`);
  ...
  prompts.outro(`🛟  Need help?
   Chat: https://enpitech.dev
   Docs: https://docs.enpitech.dev`);
}
