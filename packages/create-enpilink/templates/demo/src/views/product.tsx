...
import { useOpenExternal, useRequestModal, useRequestSize } from "enpilink/web";
      ...
      <Frame>
        ...
        <Card>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button
              variant="secondary"
              onClick={() => modal.open({ params: { id: p.id } })}
            >
              Enlarge (modal)
            </Button>
            ...
            <Button
              variant="ghost"
              onClick={() =>
                openExternal(`https://shop.northwind.example/p/${p.id}`)
              }
            >
        ...
        {expanded ? (
          <Card>
            <SectionTitle
              title="Specs"
              subtitle="This panel grows the view via useRequestSize."
            />
        ...
        {modal.isOpen ? (
          <Card className="border-primary">
            <SectionTitle
              title={`${p.name} — enlarged`}
              subtitle="Rendered in a host modal."
            />
            <div
