# enpilink Boilerplate

A minimal TypeScript boilerplate for building MCP and ChatGPT Apps with the [enpilink](https://docs.enpitech.dev) framework.
...
- Register a tool in `src/server.ts` with a unique name (e.g., `my-view`) using [`registerTool`](https://docs.enpitech.dev/api-reference/register-tool) and a `view` config.
...
The simplest way to deploy your app is by running the `deploy` command, which will push your MCP server to the [enpitech](https://enpitech.dev/) cloud for free.
