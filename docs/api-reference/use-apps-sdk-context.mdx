...
The `useAppsSdkContext` hook is a low-level hook that subscribes to global state values exposed by the OpenAI host. This hook powers many of the other enpilink hooks and can be used directly when you need access to specific global values.
...
```tsx
import { useAppsSdkContext } from "enpilink/web";
