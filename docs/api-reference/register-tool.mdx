...
<Note>
**When [built-in auth](/api-reference/mcp-server#built-in-end-user-auth-opt-in) is enabled, `securitySchemes` is enforced automatically.** A tool declaring `{ type: "oauth2", scopes: [...] }` returns **401** (no token) or **403** `insufficient_scope` (token missing a scope) — with a `WWW-Authenticate` challenge — before the handler runs. A `{ type: "noauth" }` tool runs without a token even when auth is on (the seam for guest/public access on the same `/mcp` endpoint). See the [Authentication guide](/guides/auth).
</Note>

<Warning>
When built-in auth is **off** (the default), this field is client-facing metadata only. **Servers must then enforce auth on each call** — verify `extra.authInfo` and check `authInfo.scopes` in the handler (or use a tool-call middleware). The declaration is not self-enforcing without built-in auth or your own middleware.
</Warning>
...
`content` can be a plain string, a single `ContentBlock`, or an array of `ContentBlock`s. See the different [content helpers](#content-helpers) that enpilink provides.
...
- `extra._meta` — client-supplied hints injected by the host (see [Client hints on `_meta`](#client-hints-on-_meta))

### Client hints on `_meta`

ChatGPT may inject context about the current conversation in tool calls. enpilink types these fields on `extra._meta` so you can use them in the handler:
...
<Info>
**viewUUID**: enpilink automatically injects a unique `viewUUID` into `_meta` for every tool with view call. This enables `useViewState` and `createStore` to persist state across re-renders in MCP Apps via `localStorage`. Any `_meta` you return is preserved alongside the injected `viewUUID`.
</Info>
...
`enpilink/server` exports helpers for building `ContentBlock`s:

```typescript
import { audio, embeddedResource, image, resourceLink, text } from "enpilink/server";
...
<Note>
enpilink automatically includes your server's domain in the CSP. You only need to configure additional external domains.
</Note>
