...
The `useMcpAppContext` hook is a low-level hook that subscribes to context values from the MCP Apps host. This hook powers many of the other enpilink hooks when running in MCP Apps clients and can be used directly when you need access to specific context values.
...
```tsx
import { useMcpAppContext } from "enpilink/web";
