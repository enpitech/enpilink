...
| `ENPILINK_AUTH_SIGNING_KEY` | `auth.signingKey` | **Secret** — env-only, masked, never DB-persisted. When set, enpilink becomes a **federating Authorization Server** that mints + signs its own tokens (Ed25519, key derived from this secret) and enables guest mode + lazy/step-up auth. An upstream IdP is **optional**: with one, the "Sign in" login path is layered on; without one, enpilink runs **guest-only**. Without a signing key, enpilink stays in transparent-proxy mode (upstream-issued tokens) or resource-server-only mode. |
