...
| `ENPILINK_AUTH_SIGNING_KEY` | `auth.signingKey` | **Secret** — env-only, masked, never DB-persisted. Reserved for the hosted authorization server. |
| `ENPILINK_AUTH_CLIENT_SECRET` | `auth.clientSecret` | **Secret** — env-only, masked, never DB-persisted. Reserved for the upstream IdP. |

`AuthConfig` also accepts a custom `verifier` (any `OAuthTokenVerifier`) — when
set it overrides the built-in JWKS verifier.
