...
The [OpenAI Apps SDK](https://developers.openai.com/apps-sdk/) is the ChatGPT runtime layer for ChatGPT Apps, [announced in October 2025](https://enpitech.dev/blog/inside-openai-s-apps-sdk-how-to-build-interactive-chatgpt-apps-with-mcp). ChatGPT supports the MCP Apps UI model, and also provides `window.openai` for compatibility and ChatGPT-specific extensions.
...
This API is powerful but low-level and imperative—which is where enpilink comes in.

## enpilink Hook Mapping

enpilink wraps the raw `window.openai` API with React hooks:

| Raw API | enpilink Hook | Purpose |
|---------|----------------|---------|
...
## enpilink Apps SDK Exclusive Features
...
```tsx
import { useFiles } from "enpilink/web";
...
<Warning>
ChatGPT has aggressive caching. Use the [Console](/devtools/devtools) for fast iteration, then test in ChatGPT for final validation. See [Fast Iteration](/concepts/fast-iteration).
</Warning>

## Related

- [MCP Apps](/fundamentals/mcp-apps) - The open alternative runtime
- [Write Once, Run Everywhere](/concepts/write-once-run-everywhere) - How enpilink handles both runtimes
