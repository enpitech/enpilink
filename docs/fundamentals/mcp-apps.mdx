...
description: "Deep dive into the MCP ext-apps specification: MCP protocol, enpilink hooks, and how MCP Apps clients render views."
---
...
MCP Apps are the portable baseline across hosts (e.g. Claude, Goose, VSCode, and ChatGPT compatibility mode). enpilink implements the protocol via the `McpAppAdaptor` so you use the same hooks regardless of runtime.
...
## enpilink Hook Mapping

enpilink wraps the MCP ext-apps protocol with the **same React hooks** as the Apps SDK. The `McpAppAdaptor` translates hook usage into the protocol under the hood:

| Protocol | enpilink Hook | Purpose |
|----------|----------------|---------|
...
## MCP Apps methods not yet supported in enpilink

The following [MCP ext-apps protocol methods](https://github.com/modelcontextprotocol/ext-apps) are not yet wrapped by enpilink hooks:
...
Not all enpilink features exist in the ext-apps spec. When using enpilink in MCP Apps clients, the following features are either not supported or polyfilled:
...
<Tip>
The Console's Playground uses the Apps SDK runtime. Use the Console for fast iteration, then validate in an MCP Apps client (e.g. Goose) for final testing.
</Tip>

## Related

- [Apps SDK (ChatGPT)](/fundamentals/apps-sdk) - The ChatGPT-specific runtime
- [Write Once, Run Everywhere](/concepts/write-once-run-everywhere) - How enpilink abstracts both runtimes
