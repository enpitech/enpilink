---
title: Use enpilink/web
description: "Add React hooks and utilities to your App views with enpilink/web standalone package."
---

# Use enpilink/web

This guide shows you how to use `enpilink/web` without `enpilink/server` to add React hooks and utilities to your views.
...
`enpilink/web` provides React hooks and utilities for building advanced Apps:
...
- **[`generateHelpers`](/api-reference/generate-helpers)**: Generate typed helpers for your views (requires `enpilink/server`)

Each hook is documented in the API Reference section in the sidebar.

## Basic example

Here's a simple view using enpilink hooks:

```tsx
// src/views/my-view.tsx
import { useToolInfo } from "enpilink/web";
...
If you're not using `enpilink/server`, you won't get automatic type inference. You can still add types manually:
