...
enpilink opens an account-free [srv.us](https://srv.us) SSH tunnel automatically (no login, no signup) and prints a public `/mcp` URL — e.g. `https://6x7k9m2qwerasdf.srv.us/mcp`. The URL is stable: it's derived from an ed25519 key auto-generated at `~/.enpilink/id_ed25519`, so the same machine gets the same URL every run. Add `--verbose` to stream the raw tunnel logs.
