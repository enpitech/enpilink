...
- **`restart`** — DB-editable, but the change only takes effect after a process
  **restart**. These are non-secret bootstrap keys: `port`, `storage`, `dbPath`,
  and all non-secret end-user-auth keys (`auth.enabled`, `auth.issuer`,
  `auth.audience`, `auth.jwksUrl`, `auth.upstream.*`, `auth.redirectUris`,
  `auth.branding.*`). The running process keeps using the value it booted with;
  the UI shows a **"requires restart"** badge and, once you change one, a
  "takes effect after restart" note.
- **`readonly`** — environment-only, never web-editable. These are the `admin`
  gate and the `adminAuthToken` secret.
...
| `port` | `PORT` | `3000` | restart | Server | HTTP port (restart to apply) |
| `storage` | `ENPILINK_STORAGE` | `sqlite` | restart | Storage | Storage engine — `sqlite` default (durable `enpilink.db`); `memory`/`postgres`/custom |
| `dbPath` | `ENPILINK_DB_PATH` | `./enpilink.db` | restart | Storage | SQLite database path |
...
(Analytics, Retention, Features, Display, Storage, Server, Security) rather than
a raw bootstrap/runtime split. Each row shows the human label, the raw key, the
...
unknown key → 404, secret / `admin` → 403, env-locked → 409, invalid value →
400, no storage → 409.
...
secrets, `admin`, unknown, and env-locked keys are rejected). The storage layer
backs this with `clearConfig(key, actor?)`.

## Presets

Two built-in presets bundle runtime-key changes into one action. Presets **only
touch runtime keys** — never secrets, never `admin`, never restart-tier
bootstrap keys — and **skip any key that is env/file-pinned**, reporting which
were applied vs skipped.
