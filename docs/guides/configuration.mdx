...
| `agent.ruleset.enabled` | `ENPILINK_CFG_AGENT_RULESET_ENABLED` | `true` | runtime | Agent | Fetch the [detection ruleset](/guides/agent-detection-ruleset) from the network (SWR — never blocks a request). Off → self-host / cache-only |
| `agent.ruleset.url` | `ENPILINK_CFG_AGENT_RULESET_URL` | `https://cdn.enpitech.dev/agent/ruleset/v1.json` | runtime | Agent | Ruleset artifact URL. Point at your own mirror or a `file://` path to self-host |
| `agent.ruleset.ttlSeconds` | `ENPILINK_CFG_AGENT_RULESET_TTL_SECONDS` | `0` | runtime | Agent | TTL override (s). `0` honors the artifact's `Cache-Control: max-age` |
| `agent.ruleset.timeoutMs` | `ENPILINK_CFG_AGENT_RULESET_TIMEOUT_MS` | `5000` | runtime | Agent | Hard background-fetch timeout (ms). Never blocks a response |
| `agent.ruleset.mode` | `ENPILINK_CFG_AGENT_RULESET_MODE` | `live` | runtime | Agent | `live` (long central TTL) or `dev` (short TTL to test a new signature) |
...
The `agent.ruleset.*` keys control the [detection-ruleset
fetch](/guides/agent-detection-ruleset) — the one-directional download that keeps
agent detection current without a package release. All five are runtime-editable and
**none affects request latency**: they govern only the background refresh (a request
always reads the in-memory ruleset synchronously). Turn `agent.ruleset.enabled` off,
or point `agent.ruleset.url` at your own mirror / a `file://` path, for a
self-hosted or air-gapped deployment with zero enpitech contact. See
