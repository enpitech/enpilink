...
| `agent.ruleset.enabled` | `ENPILINK_CFG_AGENT_RULESET_ENABLED` | `true` | runtime | Agent | Supply detection with a [ruleset](/guides/agent-detection-ruleset). Off → pure capture, rows stay `pending` |
| `agent.ruleset.url` | `ENPILINK_CFG_AGENT_RULESET_URL` | *empty* | runtime | Agent | Empty → the instance's own packaged ruleset, in-process (offline, no network). Set — only in the distributed case — to your own enpilink server's `/__enpilink/agents/ruleset` (or a mirror / `file://`). Never enpitech |
| `agent.ruleset.ttlSeconds` | `ENPILINK_CFG_AGENT_RULESET_TTL_SECONDS` | `0` | runtime | Agent | Distributed fetch only. TTL override (s); `0` honors the artifact's `Cache-Control: max-age` |
| `agent.ruleset.timeoutMs` | `ENPILINK_CFG_AGENT_RULESET_TIMEOUT_MS` | `5000` | runtime | Agent | Distributed fetch only. Hard background-fetch timeout (ms). Never blocks a response |
| `agent.ruleset.mode` | `ENPILINK_CFG_AGENT_RULESET_MODE` | `live` | runtime | Agent | Distributed fetch only. `live` (long TTL) or `dev` (short TTL to test a new signature) |
...
and the honest caveats — in [Agent analytics](/guides/agent-analytics) (capture /
serve / measure) and [Detection & the live
ruleset](/guides/agent-detection-ruleset) (the `agent.ruleset.*` keys). They live in
the DB and toggle live from the Configuration tab, except the secret
`agent.ingestToken`, which is env-only like the other secrets below.
</Note>

### Agent ruleset keys

The `agent.ruleset.*` keys control the [detection
ruleset](/guides/agent-detection-ruleset). enpilink is self-hosted: **by default
the instance uses its own packaged ruleset in-process** (`agent.ruleset.url` empty),
so detection works offline with **no call to enpitech** — there is no enpitech CDN.
All five are runtime-editable and **none affects request latency** (a request always
reads the in-memory ruleset synchronously; the fetch keys govern only a background
refresh in the distributed case). Set `agent.ruleset.url` **only** when a website /
edge adapter runs apart from your enpilink server — point it at *your own* server's
`/__enpilink/agents/ruleset` (or a mirror / `file://` path), never enpitech. See
[`agentCapture` &amp; the ruleset](/api-reference/agent-ruleset#config-keys) for the
per-key reference and the endpoints they drive.
...
- [Detection & the live ruleset](/guides/agent-detection-ruleset) — the `agent.ruleset.*` keys in depth
