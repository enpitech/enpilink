...
- **`restart`** — DB-editable, but the change only takes effect after a process
  **restart**. These are the non-secret bootstrap keys `port`, `storage`, and
  `dbPath`. The running process keeps using the value it booted with; the UI
  shows a **"requires restart"** badge and, once you change one, a
