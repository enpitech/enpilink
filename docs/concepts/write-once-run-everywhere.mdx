...
description: "How enpilink abstracts runtime differences so your apps work across all ChatGPT and MCP Apps hosts."
---

enpilink is designed to be **runtime-agnostic**. Your view code works the same whether it runs in MCP Apps compatible clients (like Claude, Goose, VSCode) via the MCP ext-apps specification, or in ChatGPT via the Apps SDK.
...
enpilink introduces an **adaptor layer** that normalizes these differences:
...
flowchart TB
    ...
    subgraph enpilink["enpilink Hooks"]
    ...
    style enpilink fill:#f1f5f9,stroke:#cbd5e1,stroke-width:2px,color:#334155
...
These include [useToolInfo](/api-reference/use-tool-info), [useCallTool](/api-reference/use-call-tool), and other [API hooks](/api-reference). enpilink automatically detects which runtime is available and uses the appropriate adaptor.
