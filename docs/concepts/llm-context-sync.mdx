...
enpilink uses a reserved key `__view_context` in view state. In **Apps SDK (ChatGPT)**, view state lives in `window.openai.widgetState`; in **MCP Apps**, the same key is used in the polyfilled `useViewState` store.
...
window.openai.widgetState = {
  ...
  // enpilink's context (auto-managed)
