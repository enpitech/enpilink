# Northwind Kitchen-Sink — Operator Runbook (M5)
...
1. `pnpm -F enpilink-kitchen-sink build`
2. Local: `pnpm -F enpilink-kitchen-sink dev -p 5050` → open the URL.
   Claude: `pnpm -F enpilink-kitchen-sink dev:tunnel` → add the `/mcp` URL as a
   custom connector, paste `specs/SYSTEM_PROMPT.md` into project instructions.
