...
function Account() {
  ...
  async function onUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFileNote("File upload isn't available in this runtime (Apps-SDK only).");
    }
  }
    ...
    <Frame>
      ...
      <Card>
        <div className="flex items-center gap-2">
          <SectionTitle title={output?.name ?? "Account"} subtitle={output?.email} />
      ...
      <Card>
        <SectionTitle title="Files & links" subtitle="Files degrade gracefully off Apps-SDK." />
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <Button
            variant="ghost"
            onClick={() => openExternal("https://shop.northwind.example/account")}
          >
